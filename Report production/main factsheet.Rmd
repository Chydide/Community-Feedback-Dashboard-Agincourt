...
table1 <- sqlQuery(con,"select * from cfb..Table1")%>%
  ...
  select(HouseholdCnt, Population, Male, Female, Under5, SchoolGoing)
```
...
table2 <- sqlQuery(con,"select * from cfb..PopulationByAgeGrp 
order by Ordered")%>%
  filter(Years == as.numeric(params$year) +1, VillNo== params$village) %>%
...
fig1 <- sqlQuery(con,"select * from cfb..PopulationByAgeGrp")%>%
  filter(Years == as.numeric(params$year) +1, VillNo== params$village)
...
table7 <- sqlQuery(con,"select * from cfb..freq where Category in ('MaleOutMig','FemaleOutMig', 'TotalOutMig')")%>%
  filter(Years == params$year, VillNo== params$village) %>%
  select(Category, Count)
levels(table7$Category) <- c("Female Out-Migration","Male Out-Migration","Total Out-Migration")
...
ggplot(fig2, aes(x= Years , y= CrudeRate)) +
