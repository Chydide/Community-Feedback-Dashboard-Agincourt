...
#Renaming the Value labels from english to Tsonga
names(currentedu)[5] <- "Nhlayo ya vanhi"
currentedu$Education_Level <- as.character(currentedu$Education_Level)
currentedu[currentedu == "Pre School"]<-"Xikolo xalehansi"
currentedu[currentedu == "Grade 1 to 4"]<-"Gireyedi ya 1-4"
currentedu[currentedu == "Grade 5 to 7"]<-"Gireyedi ya 5-7"
currentedu[currentedu == "Grade 8 to 12"]<-"Gireyedi ya 8-12"
currentedu[currentedu == "Incomplete tertiary"]<-"Ava hetisanga tidyondzo ta xikolo xahenhla"
currentedu[currentedu == "Complete tertiary"]<-"Va hetisile tidyondzo ta xikolo xale henhla"
...
currentedu %>%
  filter(Age > 9 & Education_Level == "Gireyedi ya 8-12") %>%
  dplyr::select(Age, Number,'Nhlayo ya vanhi') %>%
  gather(Category,Value,-c(Age)) %>%
  mutate(Category =fct_relevel(Category,"Nhlayo ya vanhi","Number")) -> t1

levels(t1$Category) <- c("nhlayo ya vanhi","Gireyedi ya 8-12")
  ...
  labs(title= "Vantshwa emigangeni lava dyondzaka Gireyedi ya 8-12",
       x = 'Malembe',
       y = 'Nhlayo ya vadyondzi') -> p1

currentedu %>%
  filter(Age > 15 , Education_Level == "Ava hetisanga tidyondzo ta xikolo xahenhla") %>%
  select(Age, Number,'Nhlayo ya vanhi') %>%
  gather(Category,Value,-c(Age)) %>%
  mutate(Category =fct_relevel(Category,"Nhlayo ya vanhi","Number")) -> t2
levels(t2$Category) <- c("Nhlayo ya vanhi","xikolo xale henhla")
  ...
  theme(legend.title.align=0.5,
        ...
        legend.title = element_blank(),
        legend.position="bottom") +
  labs(title= "Vantshwa emigangeni lava dyondzaka  xikolo xale henhla",
       x = 'Malembe',
